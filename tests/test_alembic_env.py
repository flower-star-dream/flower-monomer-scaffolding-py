...
@Description: 验证脚手架 alembic/env.py 接入框架 load_env_file 后的数据库 URL 读取链路：
              .env 中的 DATABASE_URL 能被自动加载（迁移命令单独执行时不依赖 shell
              预先导出环境变量），进程/容器环境变量优先（.env 不覆盖），
              缺省 .env 文件时安全兜底不抛异常。复用框架配置加载能力，与框架同款测试保持一致性。
"""
import os
...
def _resolve_url() -> str | None:
    """模拟脚手架 alembic/env.py _resolve_url() 的数据库 URL 读取链路：load_env_file 后读 DATABASE_URL"""
